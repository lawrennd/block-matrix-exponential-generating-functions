...
\label{rem:time_scaling}
We normalize all integrals to the time interval $[0,1]$ for notational simplicity. For physical evolution over time $t$, replace $H \to tH$ and $V \to tV$; then $L(tH, tV) = t \cdot L(H, V)$. Higher-order terms scale as $t^k$ for $k$th order. Throughout this paper we set $t=1$ without loss of generality.
\end{remark}
...
\begin{remark}[Terminology: ``causal'']
We use ``causal'' to mean a single fixed time ordering $s_1 < s_2 < \cdots < s_k$, where earlier perturbations precede later ones. This corresponds to retarded (causal) Green's functions in quantum field theory, as opposed to the time-symmetric combinations that arise in equilibrium correlators or Fr\'echet derivatives.
\end{remark}
...
The symmetric construction generalizes to higher orders. For the $n$th-order symmetric derivative, the Higham-style construction uses a block matrix of dimension $2^n n \times 2^n n$, with a block structure that encodes all $n!$ time orderings.

\begin{proposition}[Dimension scaling for $n$th-order symmetric derivative]
\label{prop:dimension_scaling}
The binary-tree construction for computing the symmetric $n$th Fr\'echet derivative $D^n \exp(H)[E_1, \ldots, E_n]$ using a block exponential yields a matrix of dimension $2^n n \times 2^n n$. Whether more compact constructions exist for general symmetric derivatives remains an open question.
\end{proposition}
...
\subsection{General Principle: Block Topology Determines Time Orderings}
\label{subsec:block_topology_principle}

We now state a general principle that unifies the constructions in \Cref{sec:first_order,sec:second_order_symmetric,sec:second_order_causal} and provides a recipe for designing block matrices to compute arbitrary time-ordered integrals.

\begin{definition}[Block DAG structure]
\label{def:block_dag}
Let $G = (V, E)$ be a directed acyclic graph (DAG) with vertex set $V = \{1, 2, \ldots, N\}$ and edge set $E \subseteq V \times V$. A \emph{block matrix with DAG structure $G$} is an $Nn \times Nn$ block matrix $M$ of the form:
\[
M_{ij} = \begin{cases}
H & \text{if } i = j, \\
E_{ij} & \text{if } (i,j) \in E, \\
0 & \text{otherwise},
\end{cases}
\]
where $H, E_{ij} \in \C^{n \times n}$, with $H$ representing the ``free evolution'' and each $E_{ij}$ a ``perturbation'' from block $i$ to block $j$.
\end{definition}

\begin{theorem}[Block exponential computes path-ordered integrals]
\label{thm:block_dag_ordering}
Let $M$ be a block matrix with DAG structure $G$ as in \Cref{def:block_dag}. Then the $(i,j)$ block of $\exp(M)$ equals:
\[
[\exp(M)]_{ij} = \sum_{\pi \in \mathcal{P}_{i \to j}} \int_{\Delta_\pi} e^{(1-s_k)H} E_{\pi_k} e^{(s_k - s_{k-1})H} \cdots E_{\pi_1} e^{s_1 H} \, ds_1 \cdots ds_k,
\]
where:
\begin{itemize}
\item $\mathcal{P}_{i \to j}$ is the set of all directed paths from vertex $i$ to vertex $j$ in $G$,
\item $\pi = (i = v_0 \xrightarrow{E_{\pi_1}} v_1 \xrightarrow{E_{\pi_2}} \cdots \xrightarrow{E_{\pi_k}} v_k = j)$ is a path of length $k$,
\item $\Delta_\pi = \{(s_1, \ldots, s_k) : 0 \leq s_1 \leq s_2 \leq \cdots \leq s_k \leq 1\}$ is the standard $k$-simplex (corresponding to the ordering imposed by the path).
\end{itemize}
\end{theorem}

\begin{proof}[Proof sketch]
The proof follows by induction on the power series $\exp(M) = \sum_{m=0}^\infty M^m / m!$. Each term $M^m$ corresponds to products of $m$ factors. For block upper-triangular $M$, the $(i,j)$ block of $M^m$ receives contributions only from sequences of transitions $i \to i_1 \to \cdots \to i_{m-1} \to j$ that respect the DAG structure. Each such sequence corresponds to a path in $G$ (possibly with repeated edges). The integral representation follows from the explicit expansion of matrix exponentials and the combinatorial structure of the simplex integral. A complete proof is given in \Cref{app:proof_dag_theorem}.
\end{proof}

\begin{remark}[Designing block matrices for specific orderings]
\Cref{thm:block_dag_ordering} provides a systematic recipe:
\begin{enumerate}
\item \textbf{Identify the desired time orderings}: Which subset of the $k!$ permutations of $\{E_1, \ldots, E_k\}$ should contribute?
\item \textbf{Construct a DAG}: Create vertices and edges such that paths from vertex $1$ to vertex $N$ correspond exactly to the desired orderings.
\item \textbf{Form the block matrix}: Use \Cref{def:block_dag} to construct $M$.
\item \textbf{Extract the result}: Compute $\exp(M)$ and extract the $(1,N)$ block.
\end{enumerate}
\end{remark}

\begin{example}[Recovering known constructions]
\label{ex:recover_known}
\begin{itemize}
\item \textbf{First-order (\Cref{thm:first_order})}: $G$ has two vertices with one edge $1 \to 2$ labeled $V$. There is one path, giving $L(H,V)$.
\item \textbf{Symmetric second-order (\Cref{thm:higham_symmetric})}: $G$ is a diamond: $1 \to 2 \to 4$ (via $E_1, E_2$) and $1 \to 3 \to 4$ (via $E_2, E_1$). Two paths give both orderings.
\item \textbf{Causal second-order (\Cref{thm:najfeld_havel_causal})}: $G$ is a chain: $1 \to 2 \to 3$ (via $E_1, E_2$). One path gives single ordering.
\end{itemize}
\end{example}

This principle clarifies why different block topologies yield different orderings and provides a systematic approach to computing any desired subset of time-ordered integrals.
...
\begin{remark}[Quantum Fisher information]
For quantum exponential families of the form \eqref{eq:quantum_exp_family}, this metric coincides with the Bogoliubov--Kubo--Mori (BKM) metric and the symmetric logarithmic derivative (SLD) Fisher information. The connection to the Kubo--Mori inner product is made explicit in \Cref{sec:kubo_mori}.
\end{remark}
...
Comprehensive benchmarking comparing accuracy, timing, and stability across a range of problem types (random Hermitian matrices, quantum spin systems, and quantum exponential families) is ongoing. Reference implementations and numerical experiments will be made available in the repository at:
\begin{center}
...
\subsection{Proof of Theorem~\ref{thm:block_dag_ordering}: Block DAG Structure}
\label{app:proof_dag_theorem}

The full proof of \Cref{thm:block_dag_ordering} proceeds by induction on the power series expansion and careful accounting of path multiplicities. The key steps are:

\begin{enumerate}
\item \textbf{Path enumeration}: Show that each term in $M^m$ contributing to block $(i,j)$ corresponds to a walk of length $m$ from $i$ to $j$ in the DAG $G$.
\item \textbf{Integral representation}: Prove that summing over all powers $m$ with weight $1/m!$ yields the simplex integral $\int_{\Delta_k}$ for each path of length $k$.
\item \textbf{Combinatorial factors}: Verify that the factorial weights from the exponential series match the measure on the simplex.
\end{enumerate}

A detailed exposition with complete proofs will appear in an extended version of this paper. The proof techniques follow those of \Cref{thm:first_order,thm:higham_symmetric}, generalized to arbitrary DAG structures using standard results from algebraic combinatorics.
